...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    s = sigmoid(a)
    assert 0.0 <= s <= 1.0
    assert pytest.approx(1 - s, rel=1e-5) == sigmoid(-a)
    if pytest.approx(a, abs=1e-5) == 0:
        assert pytest.approx(s, rel=1e-5) == 0.5
    increment = 1e-5
    if a > -1e6 and a < 1e6:
        assert s <= sigmoid(a + increment)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert is_close(add(sum_ls1, sum_ls2), sum_combined)
